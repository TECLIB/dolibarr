...
/**
 *   	\file       advanceddiscount/advanceddiscount_list.php
 *		\ingroup    advanceddiscount
 *		\brief      List page for advanceddiscount
 */
...
$contextpage= GETPOST('contextpage','aZ') ? GETPOST('contextpage','aZ') : 'advanceddiscountlist';   // To manage different context of search
...
$hookmanager->initHooks(array('advanceddiscountlist'));     // Note that conf->hooks_modules contains array
...
$permissiontoread = $user->hasRight('advanceddiscount', 'read');
$permissiontoadd = $user->hasRight('advanceddiscount', 'write');
$permissiontodelete = $user->hasRight('advanceddiscount', 'delete');
...
//$help_url="EN:Module_AdvancedDiscount|FR:Module_AdvancedDiscount_FR|ES:Módulo_AdvancedDiscount";
$help_url='';
$title = $langs->trans('ListOf', $langs->transnoentitiesnoconv("AdvancedDiscount"));
...
if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all && !$page)
{
	...
	$id = $obj->rowid;
	header("Location: ".DOL_URL_ROOT.'/advanceddiscount/advanceddiscount_card.php?id='.$id);
	exit;
}
...
if ($user->hasRight('advanceddiscount', 'delete')) {
...
$newcardbutton = '';
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/advanceddiscount/advanceddiscount_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail="SendAdvancedDiscountRef";
$modelmail="advanceddiscount";
$objecttmp=new AdvancedDiscount($db);
