...
/**
 * \file    core/triggers/interface_99_modAutoCreateTasks_AutoCreateTasksTriggers.class.php
 ...
 *      - interface_99_modAutoCreateTasks_MyTrigger.class.php
...
/**
 *  Class of triggers for AutoCreateTasks module
 */
class InterfaceAutoCreateTasksTriggers extends DolibarrTriggers
{
	/**
	...
	public function __construct($db)
	{
		...
		$this->family = "demo";
		$this->description = "AutoCreateTasks triggers.";
		// 'development', 'experimental', 'dolibarr' or version
		$this->version = 'development';
		$this->picto = 'autocreatetasks@autocreatetasks';
	}
	...
	{
		if (empty($conf->autocreatetasks->enabled)) return 0; // If module is not enabled, we do nothing
		...
		switch ($action) {
			...
			case 'PROJECT_VALIDATE':
				...
				foreach($tasks_label as $task_label)
				{
					...
					$task->fk_project = $projid;
					$task->ref = $defaultref;
					$task->fk_task_parent = 0;
					$task->label = $task_label;
					$task->description = $task_label;
					$task->date_c = dol_now();
					$task->date_start = dol_mktime(12,0,0,$dateomonth,$dateoday,$dateoyear);
					$task->date_end = '';
					$task->progress = 0;
					...
					if ($tmpresql)
					{
					   ...
					   if ($obj->nb == 0)
					   {
					       ...
					       $task->rang = $i;
					       $task->update($user);      // To update rang
					   }
					   else
					   {
