...
/**
 *  \file       justificativedocument_document.php
 *  \ingroup    justificativedocuments
 *  \brief      Tab for documents linked to JustificativeDocument
 */
...
$hookmanager->initHooks(array('justificativedocumentdocument','globalcard'));     // Note that conf->hooks_modules contains array
...
$title=$langs->trans("JustificativeDocument").' - '.$langs->trans("Files");
$help_url='';
...
if ($object->id)
{
	...
	dol_fiche_head($head, 'document', $langs->trans("JustificativeDocument"), -1, $object->picto);
	...
	//$permission = $user->rights->justificativedocuments->create;
	$permission = 1;
	//$permtoedit = $user->rights->justificativedocuments->create;
	$permtoedit = 1;
	$param = '&id=' . $object->id;

	//$relativepathwithnofile='justificativedocument/' . dol_sanitizeFileName($object->id).'/';
	$relativepathwithnofile='justificativedocument/' . dol_sanitizeFileName($object->ref).'/';
