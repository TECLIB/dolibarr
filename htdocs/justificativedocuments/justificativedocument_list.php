...
/**
 *   	\file       justificativedocument_list.php
 *		\ingroup    justificativedocuments
 *		\brief      List page for justificativedocument
 */
...
require_once __DIR__.'/class/justificativedocument.class.php';
...
$contextpage= GETPOST('contextpage', 'aZ') ? GETPOST('contextpage', 'aZ') : 'justificativedocumentlist';   // To manage different context of search
...
$hookmanager->initHooks(array('justificativedocumentlist'));     // Note that conf->hooks_modules contains array
...
//$help_url="EN:Module_JustificativeDocument|FR:Module_JustificativeDocument_FR|ES:Módulo_JustificativeDocument";
$help_url='';
$title = $langs->trans('ListOf', $langs->transnoentitiesnoconv("JustificativeDocuments"));
...
if ($num == 1 && ! empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all && ! $page)
{
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/justificativedocuments/justificativedocument_card.php', 1).'?id='.$id);
	exit;
}
...
$newcardbutton = dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/justificativedocuments/justificativedocument_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);
...
$topicmail="SendJustificativeDocumentRef";
$modelmail="justificativedocument";
$objecttmp=new JustificativeDocument($db);
...
while ($i < ($limit ? min($num, $limit) : $num))
{
	...
	foreach($object->fields as $key => $val)
	{
	    ...
	    if (! empty($arrayfields['t.'.$key]['checked']))
		{
			...
			elseif (in_array($val['type'], array('date','datetime','timestamp'))) print $object->showOutputField($val, $key, $object->$key, '');
