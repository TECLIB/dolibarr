...
/**
 *  \file       justificativedocument_note.php
 *  \ingroup    justificativedocuments
 *  \brief      Car with notes on JustificativeDocument
 */
...
$hookmanager->initHooks(array('justificativedocumentnote','globalcard'));     // Note that conf->hooks_modules contains array
...
$help_url='';
llxHeader('', $langs->trans('JustificativeDocument'), $help_url);
...
if ($id > 0 || ! empty($ref))
{
	...
	dol_fiche_head($head, 'note', $langs->trans("JustificativeDocument"), -1, $object->picto);
	...
	if (! empty($conf->projet->enabled))
	{
	    ...
	    if ($user->rights->justificativedocuments->creer)
	    {
	        if ($action != 'classify')
