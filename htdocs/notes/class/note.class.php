...
class Note extends Common_DB_Note
{
	...
	static function showEdit($id, $note_id, $type = 'socid', $mode='')
	{
		...
		print '<a href="'.$_SERVER['PHP_SELF'].'?action=edit_note&note_id='.$note_id.'&'.$type.'='.$id.'&mode='.$mode.'">
	...
	static function showDelete($id, $note_id, $type = 'socid', $mode='')
	{
		...
		print '<a id="supprimernote" href="'.$_SERVER['PHP_SELF'].'?action=del_note&note_id='.$note_id.'&'.$type.'='.$id.'&mode='.$mode.'">
